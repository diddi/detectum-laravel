...
@section('title', 'Vrijwilligers zoeken')

@section('content')
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold">Vrijwilligers zoeken</h1>
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-8">
        ...
        <form action="{{ route('admin.vrijwilligers.search') }}" method="GET" class="flex flex-col md:flex-row gap-4">
            <div class="flex-grow">
                ...
                <input type="text" id="stad" name="stad" value="{{ request('stad') }}" required placeholder="Voer een stad in" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-600">
            </div>
            <div class="self-end">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-bold">Zoekresultaten voor "{{ $city }}"</h2>
            <span class="text-gray-500">{{ $volunteers->total() }} vrijwilligers gevonden</span>
        </div>
        ...
        @if(count($volunteers) > 0)
            ...
            <!-- Pagination -->
            <div class="mt-6">
                {{ $volunteers->appends(['stad' => $city])->links() }}
            </div>
        @else
            <div class="text-center py-8">
                ...
                <p class="text-gray-500">Geen vrijwilligers gevonden voor "{{ $city }}".</p>
                <p class="text-gray-500 mt-2">Probeer een andere stad of controleer de spelling.</p>
            </div>
        @endif
    </div>
</div>
